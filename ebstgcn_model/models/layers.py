...
def temporal_conv_layer(x, Kt, c_in, c_out, act_func='relu'):
    ...
    if act_func == 'GLU':
        ...
        if act_func == 'linear':
            ...
            raise ValueError('ERROR: activation function "{}" is not defined.'.format(act_func))
...
def st_conv_block(x, Ks, Kt, channels, scope, keep_prob, act_func='GLU'):
    ...
    with tf.variable_scope('stn_block_{}_out'.format(scope)):
        x_o = temporal_conv_layer(x_t, Kt, c_t, c_oo)
    x_ln = layer_norm(x_o, 'layer_norm_{}'.format(scope))
...
def output_layer(x, T, scope, act_func='GLU'):
    ...
    with tf.variable_scope('{}_in'.format(scope)):
        x_i = temporal_conv_layer(x, T, channel, channel, act_func=act_func)
    x_ln = layer_norm(x_i, 'layer_norm_{}'.format(scope))
    with tf.variable_scope('{}_out'.format(scope)):
